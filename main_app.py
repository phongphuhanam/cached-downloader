...
DOWNLOAD_LOCATION = os.environ.get(
    "CACHED_LOCATION",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), ".cached"),
)
...
def download_file():
    ...
    try:
        _basename = minato.cached_path(
            url_or_filename=file_name,
            cache_root=DOWNLOAD_LOCATION,
            auto_update=bool(os.environ.get("AUTO_UPDATE", 0)),
            force_download=bool(os.environ.get("FORCE_DOWNLOAD", 0)),
            expire_days=os.environ.get("EXPIRE_DAYS", -1),
        )
